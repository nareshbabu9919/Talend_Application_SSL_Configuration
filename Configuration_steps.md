...
| Application | Type            | HTTPS Configuration                                            |
| ----------- | --------------- | -------------------------------------------------------------- |
| TAC         | Apache Tomcat   | Configure Tomcat SSL Connector                                 |
| TDC         | Embedded Tomcat | Configure Tomcat SSL Connector                                 |
...
Add or enable an HTTPS connector such as:
...
<Connector
    ...
    scheme="https"
    secure="true"
    keystoreFile="/appmnt/ssl/talend.jks"
...
```bash
/appmnt/Talend-8.0.1/tac/apache-tomcat/bin/shutdown.sh
...
server.ssl.key-store=/appmnt/ssl/talend.jks
server.ssl.key-store-password=changeit
server.ssl.key-store-type=JKS
```

Restart TDS using your normal startup/restart procedure (or the service manager if TDS is managed as a service).
...
Check that each application is listening on the HTTPS port:
...
Or access each application in a browser using `https://`.

---

## Important Notes

1. **Back up all configuration files** (such as `server.xml` and application configuration files) before making changes.
2. **Do not disable the existing HTTP connector** until HTTPS has been tested successfully.
3. If your applications are accessed through an external **Apache HTTP Server**, **Nginx**, or a **load balancer**, you may instead terminate SSL there and keep the backend applications on HTTP. In that case, the configuration steps are different.
4. Use the **same JKS keystore** for TAC, TDC, TDS, and TDP if they all use the same server certificate and hostname (for example, `*.cbe.com.et`).
